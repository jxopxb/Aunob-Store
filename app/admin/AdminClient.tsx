...
export default function AdminClient({
        ...
        <table className="w-full text-left border-collapse min-w-[560px]">
          <thead>
            <tr className="border-b border-zinc-900 bg-[#16191E]">
              <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500">ประเภท</th>
              <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500">รายละเอียด</th>
              <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500">วันที่</th>
              <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500 text-right">จำนวนเงิน</th>
              <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500 text-center">ลบ</th>
            </tr>
          </thead>
            ...
            {transactions.map((t) => {
              ...
              return (
                <tr key={t.id} className="hover:bg-zinc-900/20 transition-colors group">
                  ...
                  <td className="py-3.5 px-5 text-xs text-zinc-300 font-light">{t.description || "—"}</td>
                  <td className="py-3.5 px-5 text-[11px] text-zinc-500 font-light whitespace-nowrap">
                    {new Date(t.createdAt).toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "2-digit" })}
                  </td>
                  <td className={`py-3.5 px-5 text-xs font-mono text-right font-light whitespace-nowrap ${isInflow ? "text-emerald-400" : "text-rose-400"}`}>
                  ...
                  <td className="py-3.5 px-5 text-center">
                    <button
                      ...
                      className="text-zinc-700 hover:text-rose-400 p-1.5 rounded transition-colors md:opacity-0 group-hover:opacity-100"
                    >
                      <Trash2 size={13} />
                    </button>
                  </td>
  ...
  return (
    <div className="flex min-h-screen bg-[#0B0C0E] text-[#E4E6EB] font-sans antialiased w-full overflow-x-hidden">
      {/* DESKTOP SIDEBAR */}
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 md:pl-56 lg:pl-64 min-h-screen pb-10 w-full overflow-hidden">
        {/* TOP BAR */}
        <div className="sticky top-0 z-30 bg-[#0B0C0E]/90 backdrop-blur-sm border-b border-zinc-900">
          <div className="max-w-5xl mx-auto px-4 md:px-8 py-3 md:py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {/* Scrollable buttons row on mobile */}
            <div className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap pb-1 sm:pb-0 max-w-full shrink-0 -mx-4 px-4 sm:mx-0 sm:px-0">
              <button onClick={() => setIsOpenTxModal(true)} className="flex items-center gap-1.5 bg-zinc-100 text-zinc-950 hover:bg-white px-2.5 py-1.5 rounded-lg text-xs font-normal transition-all">
                <Plus size={13} />
                <span>บันทึกธุรกรรม</span>
              </button>
              <button onClick={() => setIsOpenDebtorModal(true)} className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 px-2.5 py-1.5 rounded-lg text-xs font-normal transition-all">
                <Users size={13} />
                <span>เพิ่มลูกหนี้</span>
              </button>
              <Link href="/" className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 px-2.5 py-1.5 rounded-lg text-xs font-normal transition-all">
                <ArrowLeftFromLine size={13} />
                <span>กลับหน้าหลัก</span>
              </Link>
              <button
                onClick={handleLogout}
                className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 hover:text-red-400 px-2.5 py-1.5 rounded-lg text-xs font-normal transition-all"
                title="ออกจากระบบ"
              >
                <LogOut size={13} />
        ...
        {/* PAGE CONTENT */}
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-8 space-y-6 animate-in fade-in duration-300">
          {/* ─── OVERVIEW ─── */}
          {activeView === "overview" && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={label} className={`bg-[#121418] border ${border} rounded-2xl p-4 md:p-5`}>
                    <div className="flex justify-between items-start">
                      <p className="text-[9px] md:text-[10px] tracking-widest text-zinc-500 font-light leading-relaxed">{label}</p>
                      <Icon size={14} className={iconColor} />
                    </div>
                    <p className={`text-xl md:text-2xl font-light mt-3 ${color}`}>฿{value.toLocaleString()}</p>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-[#121418] border border-zinc-900 rounded-2xl p-4 md:p-5">
                  ...
                  <div className="divide-y divide-zinc-900/40 space-y-0">
                    {initialTransactions.filter((t) => t.type === "income" || t.type === "expense").slice(0, 5).map((t) => {
                      ...
                      const displayAmount = isInflow ? t.amount : Math.abs(t.amount);
                      return (
                        <div key={t.id} className="flex justify-between items-center py-3 first:pt-0 last:pb-0">
          ...
          {activeView === "cashflow_income" && (
            <div className="space-y-4">
              <div className="bg-[#121418] border border-emerald-900/30 rounded-xl px-5 py-4 inline-flex items-center gap-3">
          ...
          {activeView === "cashflow_expense" && (
            <div className="space-y-4">
              <div className="bg-[#121418] border border-rose-900/30 rounded-xl px-5 py-4 inline-flex items-center gap-3">
          ...
          {activeView === "cashflow_monthly" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h2 className="text-sm font-light text-zinc-200 tracking-wider">สรุปรายเดือน</h2>
                <div className="relative w-full sm:w-64">
                  ...
                  <select
                    ...
                    className="w-full bg-[#121418] border border-zinc-800 rounded-xl py-2.5 pl-12 pr-10 text-xs text-white appearance-none focus:outline-none focus:border-zinc-700"
                  >
                    <option value="" disabled>เลือกเดือน</option>
                    {availableMonths.map((m) => (
                      <option key={m} value={m} className="bg-[#0F1013]">
                        {new Date(m + "-01").toLocaleDateString("th-TH", { month: "long", year: "numeric" })}
                      </option>
                    ))}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 h-3 w-3 text-zinc-500 pointer-events-none" />
                </div>
              </div>
              ...
              {loadingMonthly ? (
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {[
                      { label: "รายรับ", value: monthlyData.incomeTotal, color: "text-emerald-400", icon: ArrowUpRight, iconColor: "text-emerald-600", border: "border-emerald-900/30" },
                      { label: "รายจ่าย", value: monthlyData.expenseTotal, color: "text-rose-400", icon: ArrowDownLeft, iconColor: "text-rose-600", border: "border-rose-900/30" },
                      { label: "กำไรสุทธิ", value: monthlyData.profit, color: "text-zinc-100", icon: TrendingUp, iconColor: "text-zinc-500", border: "border-zinc-800/50" },
                    ].map(({ label, value, color, icon: Icon, iconColor, border }) => (
                      <div key={label} className={`bg-[#121418] border ${border} rounded-2xl p-4 md:p-5`}>
                        <div className="flex justify-between items-start">
                          <p className="text-[9px] md:text-[10px] tracking-widest text-zinc-500 font-light leading-relaxed">{label}</p>
                ...
                <div className="py-16 text-center text-zinc-600 text-xs tracking-widest border border-zinc-900/60 rounded-2xl bg-[#121418]">กรุณาเลือกเดือนเพื่อดูข้อมูล</div>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="bg-[#121418] border border-zinc-900 rounded-2xl overflow-x-auto">
                {initialDebtors.length === 0 ? (
                  <div className="py-16 text-center text-zinc-600 text-xs tracking-widest">ไม่มีรายชื่อลูกหนี้</div>
                ) : (
                  <table className="w-full text-left border-collapse min-w-[640px]">
                    <thead>
                      <tr className="border-b border-zinc-900 bg-[#16191E]">
                        <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500">ชื่อลูกหนี้</th>
                        <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500">หมายเหตุ</th>
                        <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500 text-right">ยอดหนี้ค้างชำระ</th>
                        <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500 text-center">จัดการ</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-900/60">
                      {initialDebtors.map((d) => (
                        <tr key={d.id} className="hover:bg-zinc-900/20 transition-colors group">
                          <td className="py-3.5 px-5 text-xs text-zinc-200 font-light">{d.name}</td>
                          <td className="py-3.5 px-5 text-xs text-zinc-500 font-light">{d.note || "—"}</td>
                          <td className="py-3.5 px-5 text-xs font-mono text-right text-amber-400 font-light">฿{d.totalDebt.toLocaleString()}</td>
                          <td className="py-3.5 px-5 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => openActionForDebtor(d.id, "debt_payment")} className="text-[10px] bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 px-2 py-1 rounded border border-teal-500/20 transition-colors">รับชำระ</button>
                              <button onClick={() => openActionForDebtor(d.id, "debt_add")} className="text-[10px] bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 px-2 py-1 rounded border border-amber-500/20 transition-colors">ให้ยืมเพิ่ม</button>
                              <button onClick={() => handleDeleteDebtor(d.id, d.name)} className="text-zinc-600 hover:text-rose-400 p-1 transition-colors"><Trash2 size={13} /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
