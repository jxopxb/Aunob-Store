    ...
    <div className="flex min-h-screen bg-[#0B0C0E] text-[#E4E6EB] font-sans antialiased">
      ...
      <main className="flex-1 md:pl-56 lg:pl-64 min-h-screen pb-10">
        ...
        <div className="sticky top-0 z-30 bg-[#0B0C0E]/90 backdrop-blur-sm border-b border-zinc-900">
          <div className="max-w-5xl mx-auto px-5 md:px-8 py-4 flex items-center justify-between gap-4">
            <button className="md:hidden flex flex-col gap-1 p-1.5" onClick={() => setIsSidebarOpen(true)}>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => setIsOpenTxModal(true)} className="flex items-center gap-1.5 bg-zinc-100 text-zinc-950 hover:bg-white px-3 py-1.5 rounded-lg text-xs font-normal transition-all">
                <Plus size={13} />
                <span className="hidden sm:inline">บันทึกธุรกรรม</span>
              </button>
              <button onClick={() => setIsOpenDebtorModal(true)} className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 px-3 py-1.5 rounded-lg text-xs font-normal transition-all">
                <Users size={13} />
                <span className="hidden sm:inline">เพิ่มลูกหนี้</span>
              </button>
              <Link href="/" className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 px-3 py-1.5 rounded-lg text-xs font-normal transition-all">
                <ArrowLeftFromLine size={13} />
                <span className="hidden sm:inline">กลับหน้าหลัก</span>
              </Link>
              <button
                onClick={handleLogout}
                className="flex items-center gap-1.5 border border-zinc-800 bg-zinc-900/50 text-zinc-200 hover:bg-zinc-800 hover:text-red-400 px-3 py-1.5 rounded-lg text-xs font-normal transition-all"
                title="ออกจากระบบ"
              >
                <LogOut size={13} />
        ...
        {/* PAGE CONTENT */}
        <div className="max-w-5xl mx-auto px-5 md:px-8 py-8 space-y-6 animate-in fade-in duration-300">
          {/* ─── OVERVIEW ─── */}
          {activeView === "overview" && (
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-[#121418] border border-zinc-900 rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <div key={label} className={`${bg} border ${border} rounded-2xl p-5`}>
                        ...
                        <p className={`text-2xl font-bold mt-3 ${color}`}>฿{value.toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
                      ...
                      <table className="w-full text-left">
                        <thead className="bg-white/5">
                          <tr className="text-[10px] uppercase tracking-widest text-zinc-500">
                            ...
                            <th className="py-3 px-5 text-right">จำนวนเงิน</th>
                          </tr>
                        </thead>
                            ...
                            monthlyData.transactions.map((tx) => {
                              ...
                              return (
                                <tr key={tx.id} className="hover:bg-white/5">
                                  ...
                                  <td className="py-3 px-5 text-sm text-zinc-300 truncate max-w-[200px]">{tx.description || "—"}</td>
                                  ...
                                  <td className={`py-3 px-5 text-sm font-medium text-right ${isInflow ? "text-emerald-400" : "text-rose-400"}`}>
                                    {prefix}฿{displayAmount.toLocaleString()}
                                  </td>
                                </tr>
                              );
          ...
          {activeView === "debtors_list" && (
            <div className="space-y-4">
              <div className="bg-[#121418] border border-amber-900/30 rounded-xl px-5 py-4 inline-flex items-center gap-3">
                  ...
                  <table className="w-full text-left border-collapse min-w-[500px]">
                    <thead>
                      <tr className="border-b border-zinc-900 bg-[#16191E]">
                        ...
                        <th className="py-4 px-5 text-[10px] font-medium tracking-widest text-zinc-500 text-right">ยอดค้าง</th>
                    ...
                    <tbody className="divide-y divide-zinc-900/60">
                      {initialDebtors.map((d) => (
                        <tr key={d.id} className="hover:bg-zinc-900/20 transition-colors">
                          ...
                          <td className="py-4 px-5 text-xs text-zinc-500 font-light">{d.note || "—"}</td>
                          ...
                          <td className="py-4 px-5">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => openActionForDebtor(d.id, "debt_payment")} className="flex items-center gap-1.5 text-[10px] font-light text-teal-400 border border-teal-500/20 bg-teal-500/10 hover:bg-teal-500/20 px-2.5 py-1.5 rounded-lg transition-colors whitespace-nowrap">
                                <DollarSign size={11} /> ชำระ
                              </button>
                              <button onClick={() => openActionForDebtor(d.id, "debt_add")} className="flex items-center gap-1.5 text-[10px] font-light text-rose-400 border border-rose-500/20 bg-rose-500/10 hover:bg-rose-500/20 px-2.5 py-1.5 rounded-lg transition-colors whitespace-nowrap">
                                <Plus size={11} /> เพิ่ม
                              </button>
                            </div>
                          </td>
      ...
      {isOpenTxModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0F1115] border border-zinc-900 rounded-[20px] w-full max-w-sm p-6 relative shadow-2xl">
            <div className="flex justify-between items-start mb-6">
              <div>
                <p className="text-[10px] tracking-[0.2em] text-zinc-600 uppercase mb-1">ธุรกรรม</p>
                <h2 className="text-[15px] font-light text-zinc-200 tracking-wide">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {txTypeOptions.map(({ value, label, icon: Icon, active }) => (
                    <button key={value} type="button" onClick={() => setTxForm({ ...txForm, type: value, debtorId: "" })}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-[11px] transition-all ${txForm.type === value ? active : "border-zinc-800 bg-zinc-900/40 text-zinc-500"}`}>
                      <Icon size={12} /> {label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex items-center bg-zinc-900/40 border border-zinc-800 rounded-xl px-4 h-12">
                  ...
                  <input type="number" step="0.01" required placeholder="0.00" value={txForm.amount}
                    ...
                    className="flex-1 bg-transparent text-xl font-light text-zinc-200 focus:outline-none" />
                </div>
              </div>
              <div>
          ...
          <div className="bg-[#0F1115] border border-zinc-900 rounded-[20px] w-full max-w-sm p-6 relative shadow-2xl">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <h2 className="text-[15px] font-light text-zinc-200 tracking-wide">เพิ่มรายชื่อใหม่</h2>
              </div>
